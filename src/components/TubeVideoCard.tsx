...
import type { TubeVideo } from "@/lib/mock-data";

export default function TubeVideoCard({ video }: { video: TubeVideo }) {
  return (
    <article className="group relative card-hover">
      <Link href={`/tube/${video.slug}`} className="block">
        <div className="relative aspect-video rounded-lg sm:rounded-xl overflow-hidden bg-dark-700 border border-white/5 group-hover:border-coral-500/30 transition-colors">
          ...
          <div className="absolute bottom-1.5 sm:bottom-2 right-1.5 sm:right-2 flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-dark-900/80 backdrop-blur-sm rounded text-[10px] sm:text-xs text-text-primary">
            <Clock className="w-2.5 sm:w-3 h-2.5 sm:h-3" /> {video.duration}
          </div>
          {video.isPremium && (
            <div className="absolute top-1.5 sm:top-2 right-1.5 sm:right-2 p-1 sm:p-1.5 bg-coral-500/20 backdrop-blur-sm rounded-md border border-coral-500/30">
              <Lock className="w-2.5 sm:w-3.5 h-2.5 sm:h-3.5 text-coral-400" />
            </div>
          )}
          {video.isNew && (
            <div className="absolute top-1.5 sm:top-2 left-1.5 sm:left-2 px-1.5 sm:px-2 py-0.5 bg-coral-500 rounded text-[8px] sm:text-[10px] font-bold text-white uppercase tracking-wider">Nuevo</div>
          )}
        </div>
        <div className="mt-2 sm:mt-3 px-0.5">
          <h3 className="text-xs sm:text-sm font-semibold text-text-primary group-hover:text-coral-400 transition-colors line-clamp-1">{video.title}</h3>
          <div className="flex items-center justify-between mt-1">
            <span className="text-[10px] sm:text-xs text-text-muted">{video.creatorName}</span>
            <div className="flex items-center gap-0.5 text-[10px] sm:text-xs text-text-muted">
              <Eye className="w-2.5 sm:w-3 h-2.5 sm:h-3" /> {video.views}
            </div>
          </div>
        </div>
      </Link>
